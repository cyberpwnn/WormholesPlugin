...
public class Vec4
{
	...
	/**
	 * Constructs a new Vec4 with coordinate values read from the specified double
	 * array. The specified offset must be 0 or greater, the specified length must
	 * be 1 or greater, and the array must have capacity equal to or greater than
	 * <code>offset + length</code>. Coordinates are assigned as follows:
	 * <p>
	 * <code>x = array[offset]</code><br/>
	 * <code>y
	 * = array[offset + 1]</code> if <code>length > 1</code>, otherwise
	 * <code>y=0</code><br/>
	 * <code>z = array[offset +
	 * 2]</code> if <code>length > 2</code>, otherwise <code>z=0</code><br/>
	 * <code>w = array[offset + 3]</code> if <code>length > 3</code>, otherwise
	 * <code>w=1</code>
	 * </p>
	 *
	 * @param array
	 *            the double array from which to read coordinate data.
	 * @param offset
	 *            the array starting index.
	 * @param length
	 *            the number of coordinates to read.
	 *
	 * @return a new Vec4 with coordinate values read from the specified double
	 *         array.
	 *
	 * @throws IllegalArgumentException
	 *             if the array is null, if offset is negative, if length is less
	 *             than 1, or if the array's capacity is less than
	...
	/**
	 * Constructs a new Vec4 with coordinate values read from the specified float
	 * array. The specified offset must be 0 or greater, the specified length must
	 * be 1 or greater, and the array must have capacity equal to or greater than
	 * <code>offset + length</code>. Coordinates are assigned as follows:
	 * <p>
	 ...
	 *
	 * @param array
	 *            the float array from which to read coordinate data.
	 * @param offset
	 *            the array starting index.
	 * @param length
	 *            the number of coordinates to read.
	...
	/**
	 * Constructs a new Vec4 with <code>x</code> and <code>y</code> values from the
	 * specified double array. The specified offset must be 0 or greater, and the
	 * array must have capacity equal to or greater than <code>offset +
	 ...
	 *            the array starting index.
	 *
	 * @return a new Vec4 with <code>x</code> and <code>y</code> coordinate values
	 *         from the specified double array.
	 *
	 * @throws IllegalArgumentException
	 *             if the array is null, if offset is negative, or if the array's
	 *             capacity is less than <code>offset + 2</code>.
	 */
	...
	/**
	 * Constructs a new Vec4 with <code>x</code>, <code>y</code> and <code>z</code>
	 * values from the specified double array. The specified offset must be 0 or
	 * greater, and the array must have capacity equal to or greater than
	 * <code>offset + 3</code>. Coordinates are assigned as follows:
	 * <p>
	 * <code>x = array[offset]</code><br/>
	 * <code>y =
	 * array[offset + 1]</code><br/>
	 * <code>z = array[offset + 2]</code>
	 * </p>
	 *
	 * @param array
	 *            the double array from which to read coordinate data.
	 ...
	 * @throws IllegalArgumentException
	 *             if the array is null, if offset is negative, or if the array's
	 *             capacity is less than <code>offset + 3</code>.
	 */
	...
	/**
	 * Constructs a new Vec4 with <code>x</code>, <code>y</code>, <code>z</code> and
	 * <code>w</code> values from the specified double array. The specified offset
	 * must be 0 or greater, and the array must have capacity equal to or greater
	 * than <code>offset + 4</code>. Coordinates are assigned as follows:
	 * <p>
	 * <code>x =
	 * array[offset]</code><br/>
	 ...
	 *            the array starting index.
	 *
	 * @return a new Vec4 with <code>x</code>, <code>y</code>, <code>z</code> and
	 *         <code>w</code> coordinate values from the specified double array.
	 *
	 * @throws IllegalArgumentException
	 *             if the array is null, if offset is negative, or if the array's
	 *             capacity is less than <code>offset + 4</code>.
	 */
	 ...
	 * <code>array[offset] =
	 * x</code><br/>
	 * <code>array[offset + 1] = y</code> if <code>length > 1</code>, otherwise
	 * <code>array[offset +
	 * 1]</code> is not written to<br/>
	 * <code>array[offset + 2] = z</code> if <code>length > 2</code>, otherwise
	 * <code>array[offset + 2]</code> is not written to<br/>
	 * <code>array[offset + 3] = w</code> if <code>length >
	 * 3</code>, otherwise <code>array[offset + 3]</code> is not written to
	 * </p>
	 *
	 * @param array
	 *            the double array to receive the coordinate data.
	 * @param offset
	 *            the array starting index.
	 * @param length
	 *            the number of coordinates to write.
	 *
	 ...
	 *
	 * @param array
	 *            the float array to receive the coordinate data.
	 * @param offset
	 *            the array starting index.
	 * @param length
	 *            the number of coordinates to write.
	...
	/**
	 * Writes this Vec4's <code>x</code> and <code>y</code> values to the specified
	 * double array. The specified offset must be 0 or greater, and the array must
	 * have have capacity equal to or greater than <code>offset + 2</code>.
	...
	/**
	 * Writes this Vec4's <code>x</code>, <code>y</code> and <code>z</code> values
	 * to the specified double array. The specified offset must be 0 or greater, and
	 ...
	 * greater than <code>offset + 4</code>. Coordinates are written to the array as
	 * follows:
	 * <p>
	 * <code>array[offset] =
	 * x</code><br/>
	 ...
	 * suitable for use where four-dimensional homogeneous coordinates are required.
	 * The returned vector has <code>x y z</code> coordinates are equal to this
	 * vector's <code>x y z</code> coordinates, and has <code>w</code> coordinate
	 * equal to 1.0.
	 * <p/>
	 * A three-dimensional point in homogeneous coordinates is necessary when
	 * transforming that point by a 4x4 transformation matrix, or when calculating
	 * the dot product of the point and the equation of a plane. The returned vector
	 * is affected by the translation component of a 4x4 transformation matrix.
	 *
	...
	{
		// For a discussion of homogeneous coordinates, see "Mathematics for 3D Game
	...
	/**
	 * Returns a representation of this vector as an <code>x y z</code> direction
	 * suitable for use where four-dimensional homogeneous coordinates are required.
	 * The returned vector has <code>x y z</code> coordinates are equal to this
	 * vector's <code>x y z</code> coordinates, and has <code>w</code> coordinate
	 * equal to 0.0.
	 * <p/>
	 * A three-dimensional direction in homogeneous coordinates is necessary when
	 * transforming that direction by a 4x4 transformation matrix. The returned
	 * vector is not affected by the translation component of a 4x4 transformation
	...
	public final Angle angleBetween3(Vec4 vec4)
	{
		...
		// The normalized dot product should be in the range [-1, 1]. Otherwise the
		// result is an error from floating
		// point roundoff. So if a_dot_b is less than -1 or greater than +1, we treat it
	...
	/**
	 * Compute the angle and rotation axis required to rotate one vector to align
	 * with another.
	 *
	 * @param v1
	 *            The base vector.
	 * @param v2
	 *            The vector to rotate into alignment with <code>v1</code>.
	 * @param result
	 *            A reference to an array in which to return the computed axis. May
	 *            not be null.
	 *
	...
	{
		// For the first vector, use the direction of the least component of this, which
		// indicates the more
	 ...
	 *
	 * @param points
	 *            the Iterable of points which define the returned arithmetic mean.
	 *
	 * @return the arithmetic mean point of the specified points Iterable, or null
	 *         if the Iterable is empty or contains only null points.
	 *
	 * @throws IllegalArgumentException
	...
	/**
	 * Calculate the extrema of a given array of <code>Vec4</code>s. The resulting
	 * array is always of length 2, with the first element containing the minimum
	 * extremum, and the second containing the maximum. The minimum extremum is
	 * composed by taking the smallest x, y and z values from all the
	 * <code>Vec4</code>s in the array. These values are not necessarily taken from
	 * the same <code>Vec4</code>. The maximum extrema is composed in the same
	 * fashion.
	 *
	 * @param points
	 *            any array of <code>Vec4</code>s
	 *
	 * @return a array with length of 2, comprising the most extreme values in the
	 *         given array
	 *
	 * @throws IllegalArgumentException
	 *             if <code>points</code> is null
	 */
