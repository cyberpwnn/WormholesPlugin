...
public abstract class BaseProvider implements PortalProvider
{
				...
				public void run()
				{
					...
					PlayerHud hud = new PlayerHud(p, true)
					{
						...
						public void onSelect(String selection, int slot)
						{
							...
							if(s.startsWith("Command Script"))
							{
								NMSX.sendActionBar(p, C.YELLOW + "Run commands on portal entry");
							}
						...
						public String onEnable(String s)
						{
							...
							else if(s.startsWith("Command Script: "))
							{
								s = "Command Script: " + l.getSettings().getConfig();
							}
						...
						public void onClick(Click c, Player p, String selection, int slot, Hud h)
						{
							...
							if(selection.startsWith("Command Script: "))
							{
								close();

								Hud scriptList = new PlayerHud(p, true)
								{
									@Override
									public void onUpdate()
									{
									...
									public void onClick(Click c, Player p, String selection, int slot, Hud h)
									{
										new GSound(MSound.WOOD_CLICK.bukkitSound(), 0.3f, 0.8f).play(p);
										close();
										l.getSettings().setConfig(selection);
										l.save();
										configure(l, p);
									}
								};
								...
								GList<String> opv = new GList<String>();
								opv.add(TXT.line(C.GOLD, 5) + C.GRAY + " Scripts " + TXT.line(C.GOLD, 5));
								opv.add("none");

								for(String i : Wormholes.instance.scripts.k())
								{
									opv.add(i.toString());
								}

								scriptList.setContents(opv);
								((BaseHud) scriptList).setHasTitle(true);
								scriptList.open();
							}
	...
	private void handleHud(PlayerHud h, boolean d, LocalPortal l)
	{
		...
		if(d)
		{
			...
			op.add("Command Script" + ": " + l.getSettings().getConfig());
