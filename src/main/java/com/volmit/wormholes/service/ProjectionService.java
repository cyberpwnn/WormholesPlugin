...
public class ProjectionService implements Listener {
	...
	private void startProjection(Portal po) {
		if (po.getPosition().getArea().hasPlayers() && ((LocalPortal) po).getSettings().isProject()
				&& ((LocalPortal) po).getMask().needsProjection()) {
		...
		if (p.hasWormhole()) {
			...
			if (plane.hasContent()) {
				...
				for (Player i : view.k()) {
					...
					Viewport vOut = lastPort.containsKey(p) && lastPort.get(p).containsKey(i) ? lastPort.get(p).get(i)
							: new NulledViewport(i, p);
	...
	private void renderStage(Player player, Portal portal, GMap<Vector, MaterialBlock> dimension, Viewport dialate, Viewport erode)
	{
		...
		lock.getVRM(portal, player).render();
	}
	...
	private void queueViewOut(Viewport vOut, Viewport vIn, Player i) {
	...
	private void queueViewIn(Viewport vIn, Portal p, PortalIdentity identity, GMap<Vector, MaterialBlock> map,
			Player i) {
		Wormholes.pool.queue(new Execution() {
			@Override
			public void run() {
